...
- [P2] `kb archive --stale` move artigos cuja idade em dias (mtime) exceda o threshold global `stale_days` reportado pelo health summary (valor numérico em dias de inatividade; aplica-se uniformemente a todos os artigos)
...
- Stale threshold lido via `kb.analytics.health.get_health_summary()["stale_days"]`
...
- `--stale`: filtra artigos com stale_days acima do threshold global
...
- Unit: `test_archive_stale_uses_threshold` — mock de health summary com stale_days=5 (threshold global), artigo com mtime > 5 dias é considerado stale e movido
...
- `--stale` usa `stale_days` do health summary como threshold global (em dias); o mesmo valor se aplica a todos os artigos, não é um campo por artigo.
