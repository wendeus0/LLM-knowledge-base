...
def get_health_summary() -> dict:
    ...
    return {
        ...
        "stale_days": _STALE_THRESHOLD_DAYS,
...
def evaluate_health_thresholds(
    ...
    violations: list[str] = []
    if stale_max_pct is not None and float(summary.get("stale_pct", 0.0)) > float(
        stale_max_pct
    ):
        violations.append(
            f"stale_pct={summary.get('stale_pct')} ultrapassou limite={stale_max_pct}"
        )
    if disputed_max_pct is not None and float(summary.get("disputed_pct", 0.0)) > float(
        disputed_max_pct
    ):
        violations.append(
