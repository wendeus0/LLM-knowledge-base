...
def test_should_list_articles_with_slug_title_and_topic(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    corpo = api_client.get("/articles").json()
...
def test_should_exclude_derived_files_from_the_listing(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    slugs = {a["slug"] for a in api_client.get("/articles").json()["results"]}

    assert not any(s.startswith("_") for s in slugs)


def test_should_filter_by_topic_for_the_sidebar(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    corpo = api_client.get("/articles", params={"topic": "cybersecurity"}).json()

    assert {a["slug"] for a in corpo["results"]} == {"cybersecurity/dorks", "cybersecurity/xss"}


def test_should_honour_the_limit(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    assert len(api_client.get("/articles", params={"limit": 2}).json()["results"]) == 2


def test_should_sort_by_recency_by_default(tmp_path, monkeypatch, api_client):
...
def test_should_sort_by_title_when_asked(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    titulos = [a["title"] for a in api_client.get("/articles", params={"sort": "title"}).json()["results"]]

    assert titulos == sorted(titulos)


def test_should_reject_an_unknown_sort(tmp_path, monkeypatch, api_client):
    _vault(tmp_path, monkeypatch)

    assert api_client.get("/articles", params={"sort": "aleatorio"}).status_code == 422
