...
class TestArticleExpoeWikilinks:
    def test_should_expose_wikilinks_so_the_reader_can_navigate(self, tmp_path, monkeypatch):
        _vault(tmp_path, monkeypatch)

        corpo = _client().get("/article/cybersecurity/dorks").json()
    ...
    def test_should_resolve_each_wikilink_to_a_slug(self, tmp_path, monkeypatch):
        _vault(tmp_path, monkeypatch)
    ...
    def test_should_flag_an_ambiguous_wikilink(self, tmp_path, monkeypatch):
        _vault(tmp_path, monkeypatch)

        por_texto = {
            w["text"]: w for w in _client().get("/article/cybersecurity/dorks").json()["wikilinks"]
        }
    ...
    def test_should_report_a_wikilink_without_article_as_unresolved(self, tmp_path, monkeypatch):
        wiki = _vault(tmp_path, monkeypatch)
        (wiki / "cybersecurity" / "dorks.md").write_text(
            "---\ntitle: D\n---\nVeja [[nao-existe]].\n", encoding="utf-8"
        )

        alvo = _client().get("/article/cybersecurity/dorks").json()["wikilinks"][0]
...
class TestSearchApresentavel:
    def test_should_return_a_title_for_each_result(self, tmp_path, monkeypatch):
        _vault(tmp_path, monkeypatch)

        resultados = _client().get("/search", params={"q": "dorks", "top_k": 3}).json()["results"]

        assert resultados
        assert all(r.get("title") for r in resultados)

    def test_should_return_the_topic_for_each_result(self, tmp_path, monkeypatch):
        _vault(tmp_path, monkeypatch)
