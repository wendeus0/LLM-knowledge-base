...
def test_doc_gate_should_fail_when_git_diff_fails(monkeypatch):
    monkeypatch.setattr(
        "subprocess.run",
        lambda *args, **kwargs: type(
            "Proc", (), {"stdout": "", "stderr": "fatal: bad revision", "returncode": 128}
        )(),
    )

    result = runner.invoke(app, ["jobs", "doc-gate"])
    assert result.exit_code == 1
    assert "Falha ao calcular diff" in result.stdout
