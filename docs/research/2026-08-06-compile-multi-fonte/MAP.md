...
- [Qualidade da proveniência do manifest](008-qualidade-da-proveniencia.md) — a proveniência por basename **sustenta** o agrupamento: 0 falso positivo em 40 pares (teto de 7,5%), 94,8% dos basenames únicos, e a cadeia omite em vez de errar. O gargalo é cobertura (35% dos vivos fora), não precisão.
...
- E14. Colisão de basename entre as três raízes de fonte, medida em 2026-08-06 (`008-extract.py`): 1.517 arquivos, 1.416 basenames distintos, **1.342 únicos (94,8%)**, 74 colidentes. As colisões grandes são paratexto de conversão de ebook — `03-preface.md` e `01-cover.md` (6× cada), `01-contents.md` (5×), `20-index.md` (4×). O risco residual é estrutural e confinado a uma classe de arquivo identificável pelo nome.
- E15. **O acervo tem livro duplicado.** *Mathematics for Machine Learning* existe com nome idêntico em `library/software-engineering/` e em `wiki/_sources/` (20 capítulos em cada); *Designing ML Systems* aparece nas duas raízes (19 e 5); *Observability Engineering* tem duas pastas em `library/` (36 e 8). E há **artigo duplicado na wiki viva**: `honeycomb.md` existe na raiz e em `cybersecurity/`, com conteúdos diferentes. Consequência direta para [de-onde-a-sintese-le](003-de-onde-a-sintese-le.md): ler `library/` + `_sources/` é ler livro repetido.

## Not yet specified
A **higiene do acervo** entrou na névoa por E15 e não estava no charting: livros duplicados entre `library/` e `_sources/`, duas edições do mesmo livro dentro de `library/`, e pelo menos um artigo duplicado na wiki viva. Não dá para ticketar antes de [de-onde-a-sintese-le](003-de-onde-a-sintese-le.md), porque o que conta como duplicata depende de qual raiz a síntese lê — mas a decisão de corpus não é honesta sem esse número.

Os 88 `unresolved` de `learning` que apontam para transcrições de YouTube podem ser **renomeação recuperável** em vez de fonte ausente: os identificadores de playlist aparecem com e sem prefixo de autor e com IDs truncados de formas diferentes. A medição de 008 não separou as duas hipóteses, e o tamanho do resgate é desconhecido.

O destino dos 120 `unresolved` quando o tema nascer segue em aberto — se entram como fonte elegível, se ficam de fora, se exigem rodada de desempate antes do piloto. A extração dos 23 binários da `library/` (incluindo a categoria `llm/` inteira, que não tem um markdown sequer) é trabalho real que o map pressente mas não consegue ticketar antes de se saber de onde a síntese lê. A reativação do min-refs (decisão 1 do ADR: "artigo raso é bug") paira sobre o gate novo: não está claro o que conta como referência real num artigo costurado de várias fontes, nem como os dois gates convivem. A mecânica de "livro novo marca o tema como stale" (decisão 4 do ADR) não está desenhada — nem detecção, nem superfície, nem re-compile. E há custo e latência de síntese por resolver: *Release It!* tem 138 capítulos e não cabe num prompt; qualquer destino realista passa por seleção ou redução de contexto que ainda não existe.
