...
| P3 | **Dois checkouts divergentes do repo** | `~/dev/github.com/wendeus0/` está à frente mas **sem `.env`** — rodar `kb` de lá aponta `KB_DATA_DIR` para o próprio repo (`kb/config.py:10`), a falha silenciosa de `ERROR_LOG.md:30`. `~/dev/personal/` tem `.env` e `.venv` mas está atrás. Decidir o canônico e reconciliar | 2026-07-30 |

## Sessão 2026-07-31 — gate de saída do compile

| Prioridade | Item | Contexto | Data |
| --- | --- | --- | --- |
| P2 | **`tests/unit/test_diff.py::test_diff_error_message_should_escape_rich_markup` falha na branch atual** | Pré-existente, confirmado por `git stash` das mudanças do dia: falha igual com e sem elas. O teste espera `'[bold]injetado[/bold]'` literal em `result.output`, mas o Rich renderiza a markup escapada com códigos ANSI. Não tem relação com o compile | 2026-07-31 |
