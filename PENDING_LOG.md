...
| P3 | **`localhost` remapeado em `/etc/hosts` escapa do teste de loopback** | `is_loopback()` decide por nome antes de resolver: `localhost` apontando para IP remoto seria isento. Achado do review do PR #63. **Não corrigido por escolha:** resolver o host a cada chamada custa DNS no caminho quente, introduz TOCTOU (resolver ≠ conectar) e o modelo de ameaça é fraco — quem edita `/etc/hosts` também edita o `.env`, o código e o próprio gate. A mitigação que vale já existe: `local_http_client()` com `trust_env=False` para os clientes de loopback. Reabrir se o kb passar a rodar em ambiente multi-tenant ou container compartilhado | 2026-08-01 |
| P3 | **`--allow-sensitive` não alcança embeddings e rerank** | A cadeia `compile → refresh_embeddings_index → build_index → embed_texts` não carrega o parâmetro. Opt-in provisório por `KB_EGRESS_ALLOW_SENSITIVE=1` (testado). Threading do parâmetro é a correção certa; exige mudar 4 assinaturas | 2026-08-01 |

## Sessão 2026-08-02 — a plataforma como detector de problema do corpus

| Prioridade | Item | Contexto | Data |
| --- | --- | --- | --- |
| P2 | **Dedup dos 59 pares com cosseno ≥ 0,95 sobe de prioridade** | O usuário abriu a plataforma e viu os duplicados em segundos, sem medir nada: "Reconhecimento **por** mecanismos de busca" e "Reconhecimento **com** mecanismos de busca" são o mesmo documento da OWASP WSTG ingerido de duas URLs (`wstg-latest-owasp-foundation.md` e `raw-githubusercontent-com-owasp-wstg-mas.md`). O map já tinha medido 59 pares assim e registrado como V5 do backlog; o que mudou é a **visibilidade** — antes exigia rodar cosseno, agora salta da tela. Causa raiz inalterada: `manifest.json` nunca materializado, então o compile não sabe que já viu a fonte e cria artigo novo em vez de atualizar | 2026-08-02 |
| P3 | **A plataforma revelou uma classe de uso que o map não previu** | Ler o corpus numa interface expõe problemas que a medição por script não priorizava: título truncado no slug (`...informaca.md`), artigo de 69 palavras ao lado de um de 850, wikilink apontando para conceito sem artigo. São os mesmos defeitos que o ticket 003 mediu, mas a ordem de incômodo é outra quando se lê em vez de medir | 2026-08-02 |

## Sessão 2026-08-05 — correção do review do PR #66 e Direção B (PR #67 mergeado)

| Prioridade | Item | Contexto | Data |
| --- | --- | --- | --- |
| P2 | **Destaque que atravessa parágrafos continua orfanando** | `render.plain_text` concatena os nós de texto sem separador de bloco; o browser serializa `\n\n` entre parágrafos. Uma seleção dentro de um bloco casa (inclusive através de ênfase, link e wikilink, que antes orfanavam); uma seleção que cruza blocos não casa e vira órfã. Não é regressão e não marca a posição errada — `_intervalos` confere o texto no offset antes de marcar. Paridade com a serialização do browser exige replicar `\n\n` entre blocos, `\n` entre linhas e as regras de lista e tabela; só vale se aparecer no uso real | 2026-08-05 |
| P3 | **`Form(...)` nativo do FastAPI não adotado em `study/web.py`** | Trocaria o `_form_value` caseiro, mas exige `python-multipart` como dependência de runtime — inclusive para `application/x-www-form-urlencoded`, que é o que o htmx envia. Ganho é cosmético e o parser atual está coberto por teste. Reabrir se algum formulário passar a precisar de upload | 2026-08-05 |
